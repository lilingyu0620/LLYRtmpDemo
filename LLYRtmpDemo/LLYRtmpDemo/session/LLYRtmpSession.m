...
@interface LLYRtmpSession ()<LLYStreamSessionDelegate>
{
    //两个线程，一个负责组装数据，一个负责发送数据
    dispatch_queue_t _packageQueue;
...
@property (nonatomic,strong) LLYStreamSession *streamSession;
...
- (LLYStreamSession *)streamSession{
    if (_streamSession == nil) {
        _streamSession = [[LLYStreamSession alloc] init];
        _streamSession.delegate = self;
    }
    return _streamSession;
}

- (void)setConfig:(LLYRtmpConfig *)config{
    _config = config;
    self.url = config.url;
}
...
- (instancetype)init
{
    ...
    if (self) {
        _rtmpStatus = LLYRtmpSessionStatusNone;
        _packageQueue = dispatch_queue_create("packet", 0);
...
- (void)didReceivedata{
    ...
    if (self.rtmpStatus >= LLYRtmpSessionStatusConnected &&
    ...
    switch (_rtmpStatus) {
        ...
        case LLYRtmpSessionStatusHandshake2:{//s2
...
- (void)sendSetChunkSize:(int32_t)newChunkSize{
    ...
    dispatch_sync(_packageQueue, ^{
        ...
        [data appendByte24:0];
        [data appendByte24:4];
        [data appendByte:LLYMSGTypeID_CHUNK_SIZE];
